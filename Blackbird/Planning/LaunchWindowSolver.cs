...
namespace Blackbird.Planning
{
    ...
    public static class LaunchWindowSolver
    {
        ...
        public static List<Candidate> Solve(Inputs input)
        {
            ...
            foreach (NodeCrossing crossing in FindPlaneCrossings(input, hHat, pole))
            {
        ...
        private struct NodeCrossing { public double Ut; public bool Ascending; }
        ...
        private static List<NodeCrossing> FindPlaneCrossings(Inputs input, Vector3d hHat, Vector3d pole)
        {
            ...
            {
                double fa = OutOfPlane(input, hHat, (i - 1) * dt);
                double fb = OutOfPlane(input, hHat, i * dt);
                double fc = OutOfPlane(input, hHat, (i + 1) * dt);
                if (fb > fa || fb > fc) continue;                       // not a local minimum

                double tc = RefineMinimum(input, hHat, (i - 1) * dt, (i + 1) * dt);
                if (OutOfPlane(input, hHat, tc) > tol) continue;        // minimum doesn't reach the plane
                ...
                bool ascending = Vector3d.Dot(Vector3d.Cross(hHat, siteHat), pole) > 0.0;
                crossings.Add(new NodeCrossing { Ut = input.CurrentUt + tc, Ascending = ascending });
            }
            return crossings;
        }

        // |sin(angle between the site direction and the orbit plane)| = |dot(siteHat, hHat)|.
        private static double OutOfPlane(Inputs input, Vector3d hHat, double dtFromNow)
            => Math.Abs(Vector3d.Dot(SitePositionInertial(input, dtFromNow).normalized, hHat));

        // Ternary search for the minimum of OutOfPlane over [lo, hi] (unimodal near a crossing).
        private static double RefineMinimum(Inputs input, Vector3d hHat, double lo, double hi)
        {
            for (int i = 0; i < 60; i++)
            {
                double m1 = lo + (hi - lo) / 3.0, m2 = hi - (hi - lo) / 3.0;
                if (OutOfPlane(input, hHat, m1) < OutOfPlane(input, hHat, m2)) hi = m2; else lo = m1;
            }
            return 0.5 * (lo + hi);
        }
